...
sudo openssl req -x509 -nodes -days 365 -newkey rsa:2048 -keyout otus.mip.ru.pem -out otus.mip.ru.pem
...
grep -v -E '^\s*#|^\s*$|^\s*\/|^\s*\*' /var/www/html/wordpress/wp-config.php | sed 's/#.*$//'
```
